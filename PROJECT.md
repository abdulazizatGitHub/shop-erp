...
**Last updated:** 2026-09-26 (Phase 16 — COMPLETE, P16-1–P16-4,
845/845 tests — see PROGRESS.md Session 86; closes BUG-COMMISSION-MULTI
as SUPERSEDED by ADR-0015. Owner-verified in the running app: P16-4's
Shop Identity checklist, the full end-to-end commission flow, and the
P16-3c technician-list lock/reason flow. **Next phase:** not yet
defined — `docs/PHASES.md`'s own written plan stops at Phase 12;
Phases 13–16 were each scoped ad hoc as they came up, with their own
`docs/phases/PHASE_N.md`. No Phase 17 exists yet; awaiting the next
owner-directed task.)
**Previous:** Phase 15 — COMPLETE, P15-1–P15-6, 644/644 tests; closes
BUG-JOBCLIENT-1
**Before that:** Phase 14 — COMPLETE, P14-1–P14-8 plus the
V1–V5/F1–F3/G1–G6 polish passes, 627/627 tests
...
automated-verified.** Baseline test count needed a real fix before any
code could be written: `npm run verify` initially failed 281/569 tests
(`TypeError: Cannot read properties of undefined (reading 'close')` in
every repository test's `afterEach`) — root cause was `better-sqlite3`'s
native binary compiled for `NODE_MODULE_VERSION 130` against a Node
runtime needing `127`, an environment issue unrelated to any prior
session's code. Fixed with `npm rebuild better-sqlite3` (no source
changes); re-confirmed 569/569 matching Phase 12's own close-out count
exactly. **Final count: 598/598 (baseline + 29, target was + 22
minimum).** `npm run typecheck`, `npm run lint`, and both
`npm run build --workspace=@shop/client`/`--workspace=@shop/server`
all exit 0.
Built: shop identity backend (`getShopIdentity`/`setShopIdentity`,
7-field Settings card) + shared `DocumentHeader`/`Footer`/`Section` UI
patterns backed by a `ShopIdentityContext` (defined in `packages/ui`,
fed by an IPC-calling provider in `apps/client`, since `packages/ui`
may never import app-layer code); a window-function customer ledger
query (`getCustomerLedger`, new `customer-ledger.repository.ts` since
`party.repository.ts` was already 568 lines) plus its contracts and
`customer:ledger` IPC channel; `sale:getWithLines` (confirmed REQUIRED
— `SaleSummaryDto` carries no line items); a full `CustomerDetailPage`
(stat cards, ledger table, back nav) replacing the old flat customer
list's inline "Record Payment" button; `SaleInvoiceModal` and
`PaymentReceiptModal` (previous/remaining balance reuses the same
window-function ledger query, so the two documents can never disagree);
a `CustomerStatementModal` with date-range PDF export
(`getCustomerStatementData`'s opening balance is a separate `SUM` query
over all pre-range rows, never derived from the window function); an
`AddCustomerModal` with a real price-level dropdown (added
`party:listPriceLevels` mid-session, owner-approved, since
`item:lookups` has no such field and none existed anywhere in the
codebase); and the Option B conversion of customer balance import
(`dialog.showOpenDialog`/`readFileSync` → renderer reads the CSV via
the File API and sends content directly, mirroring the Suppliers
redesign session's precedent exactly).
**Six deviations from the spec's own assumptions, all found and
resolved mid-build, documented in `docs/phases/PHASE_13.md` §8**: no
`packages/ui/src/patterns/index.ts` barrel exists in the real code (used
the real export-from-package-root pattern instead); `RecordPaymentModal`
had no "success state" for CL-7F's Print-receipt button to attach to
(built one); `SaleInvoiceModal` omits the discount line (`InvoiceData`
never carried `discountPaisa` through — logged, not fixed, to avoid
touching the shared print-layout types this phase); `AddCustomerModal`
has no "City / area" field (`CreateCustomerInput` doesn't have one);
CL-0a's Settings fields need a setter, not just the spec's 3 read tests,
for the "Add fields for Phone, Address..." task to actually work (added
`setShopIdentity` + 2 tests); `TableRow` gained an optional `onClick`
prop (no table row was clickable anywhere in the app before this phase).
**Not done this session**: owner visual confirmation of the six new/
changed screens in a running window — Electron GUI launch was
deliberately skipped (this project's own history shows repeated launch
failures in sandboxed environments — see Session 47/48 notes below —
and this environment is Windows, not the Linux/xvfb target the
available run-skill's Electron example covers). Full session detail:
...
**Update, 2026-09-09 (Session 45) — discount presets (owner-configured,
replaces free-form entry) COMPLETE.** Session C's free-form PKR/%
discount text inputs are gone; the owner now configures fixed preset
lists (PKR amounts and percentages) in Settings, and the salesman picks
from a dropdown built from those presets — no typed discount value on
the sale screen any more. `sale.discount_amount` (the DB column) is
unchanged; only the input method changed, confirmed by re-reading
`useSaleFlow.ts`'s `handleCheckout` before writing any code — it still
sends a single `discountPaisa` number to `sale:create` exactly as
Session 44 left it.
**Both `useSaleFlow.ts` (371 live lines, not the 356 PROJECT.md had
recorded) and `SettingsPage.tsx` (309 live lines) were confirmed over
cap by direct `wc -l`, per CLAUDE.md rule 6 (live code is the truth,
not the last-recorded number) — both split before any feature code,
per the brief's explicit prerequisite.** `useSaleFlow.ts` → 295 lines,
via two new hooks (`usePricePreview.ts`, `useSaleKeyboardShortcuts.ts`)
and a pure-function extraction (`saleWarnings.ts`) once the new
discount-dropdown props pushed it back over cap a second time.
`SettingsPage.tsx` → 23 lines, now a thin composer over four new card
components (`ShopIdentityCard.tsx`, `ReceiptSettingsCard.tsx`,
`BackupRestoreCard.tsx`, `DiscountPresetsCard.tsx`).
**Owner-directed mid-session change to the brief**: the Session 44
"Discount defaults" card (wholesale auto-prefill %/PKR) was removed
entirely rather than kept alongside the new presets — two discount
configuration surfaces in Settings would have confused whoever manages
them, and the new preset system already covers the wholesale case.
Removed end-to-end: UI card, the `useDiscount.ts` prefill effect that
called it, and the full backend (`setting.repository.ts`'s
`getWholesaleDefaultDiscountPct`/`Paisa` functions and their setters,
the two contracts, the two IPC channels, the two handlers, the preload
exposures, the `electron-api.d.ts` types) — a repo-wide grep for both
key strings (`wholesaleDefaultDiscountPct`/`Paisa`) returned zero
source hits after removal, pasted in the session transcript.
New setting keys, using the exact snake_case names the brief specified
(a deliberate deviation from this file's usual camelCase convention,
since the brief was explicit): `discount_apply_walkin`,
`discount_apply_wholesale`, `discount_pkr_enabled`,
`discount_pct_enabled`, `discount_pkr_presets`, `discount_pct_presets`
— the last two stored as JSON arrays of the owner's raw typed strings
(never paisa/percent-as-number at the repository layer). New combined
IPC read `settings:getDiscountConfig` (single call, D-2) does the only
PKR-string-to-paisa conversion in the whole feature, in the handler,
matching the brief's explicit "conversion happens in the handler, not
the renderer" instruction. `useDiscount.ts` rewritten around this:
exposes `applicable`/`pkrEnabled`/`pkrOptionsPaisa`/`pctEnabled`/
`pctOptions` plus selected-value state with mutual exclusivity
(selecting one preset resets and disables the other, matching Session
44's onChange-not-onBlur precedent) and a customer-type/`applyTo*`
applicability check (`walkin` = no customer selected; wholesale =
`customerType === 'wholesale'` — a named retail credit customer gets
no discount option under either checkbox, per the brief's literal two-
checkbox spec; flagged here since it's a real behavior change from
Session 44's free-form inputs, which had no customer-type
restriction). `CheckoutPanel.tsx`'s old PKR/% `TextInput` pair is
fully gone, replaced by up to two `<select>` dropdowns, hidden
entirely when the section isn't applicable to the selected customer.
`npm run verify` 428/428 confirmed before starting (after the routine
BUG-7 `better-sqlite3` ABI fix — `npm install better-sqlite3 --no-save`
— documented as already known, not a new bug) and after every
sub-task; `npm run build --workspace=@shop/client` and
`--workspace=@shop/server` both clean throughout.
**Real running-window verification, not just code-read**: same
session-only `playwright-core` precedent as Sessions 42–44 (not
persisted — confirmed via `git status` after cleanup), this time
connected over CDP (`chromium.connectOverCDP`) rather than
`_electron.launch`, since the latter's own launch-line detection
failed in this sandbox for a reason not fully diagnosed (Electron
rejected a `--remote-debugging-port=0` switch playwright inserts as
"bad option" — logged here rather than investigated further, out of
this session's scope) — manually spawning `electron.exe` with a fixed
`--remote-debugging-port` and attaching Playwright to that CDP
endpoint worked cleanly and is a valid fallback for any future session
hitting the same issue. All 5 scenarios from the brief run against
real dev-DB fixtures (`Compressor` Rs 6,000/Rs 4,500 wholesale,
`Khan Wholesale`): Settings save showed "Discount presets saved.",
confirmed via a direct `setting` table read (all 6 keys present,
JSON-array values exactly as typed); walk-in + Compressor showed both
dropdowns, selecting Rs 200 PKR disabled the % dropdown and dropped
the total to Rs 5,800 exactly; resetting to None and picking 5% showed
Rs 5,700 (Rs 300 discount) and completed the sale (INV-0066) — a
direct `sale` table query confirmed `discount_amount = 30000`,
`total_amount = 570000`, matching the hand-calc exactly; switching to
Khan Wholesale (wholesale not yet enabled for discount) made the whole
discount section disappear, confirmed both in the DOM text and a
screenshot; enabling "Apply to: Wholesale" in Settings and re-selecting
Khan Wholesale on a fresh cart brought the dropdowns back. Zero bugs
found this session. `better-sqlite3` rebuilt for Electron then
restored to the system-Node build before the final `npm run verify`;
`playwright-core` uninstalled; all screenshots/temp scripts deleted
...
left panel becomes a product-card grid, with the held-sale queue moved
from the topbar to a strip at the bottom of that panel).
**Three real corrections to the brief's own SQL, found before writing
any code and flagged/approved before proceeding, per CLAUDE.md rule
5**: (1) `v_stock_on_hand` is `GROUP BY (tenant_id, item_id,
warehouse_id)` — this shop has multiple warehouses (Shop/Store Room/
Van plus per-technician custody warehouses from Phase 6/7), so the
brief's literal `LEFT JOIN` would have silently duplicated search
result rows for any item moved in more than one warehouse; used a
scalar subquery instead (`SUM(qty_milli)` correlated by item+tenant,
naturally SQL `NULL` with zero rows — no `COALESCE` needed), same
aggregate-all-warehouses approach `report.repository.ts`'s
`getStockValuationReport` already established. (2) the brief's
`topSellingItems` SQL referenced `item.status = 'active'`, a column
that doesn't exist — the real column is `item.is_active` (integer
1/0). (3) the brief's SQL had no join to `sale`/no status filter at
all; `cancelSale` flips `sale.status` to `'cancelled'` without
deleting `sale_line` rows, so an unfiltered `SUM` would have wrongly
counted a cancelled sale's quantity as "sold" — added
`sale.status = 'confirmed'` to the join, and a dedicated test proving
a cancelled-only item is excluded from the ranking.
**One dead code finding, logged not fixed**: `channels.item.stockOnHand
= 'item:stockOnHand'` already existed in `channels.ts` with zero
handler, zero preload exposure, zero client usage anywhere — an
orphaned channel from an earlier phase, untouched by this session's
new (distinctly-named) `item:topSelling` channel. See BUG-23 below.
**Two owner decisions made mid-session, before writing the affected
code, since the brief conflicted with either the live code or its own
stated constraints**: (a) the brief said cards should show
wholesale-resolved prices for a selected customer "without a new IPC
call," but no such data exists until an item is actually in the cart
(`item:getPrices` is cart-line-scoped) — owner chose retail-price-only
on cards, matching the brief's own no-new-IPC-call constraint over its
visual-intent line. (b) the brief said "cart stays on the right panel
— unchanged," but the live code has always rendered `CartTable` in the
LEFT panel (with item search); right panel is customer+checkout only
— CLAUDE.md rule 6, live code is the truth. Owner chose to keep cart
in the left panel (below the new grid), left the right panel
untouched, treating the brief's line as a wording error rather than a
scope instruction to relocate `CartTable`.
**File-cap note**: `apps/server/src/preload.ts` (351 lines) and
`apps/client/src/types/electron-api.d.ts` (386 lines) were already
over the 300-line cap before this session touched them; owner chose
to add the small `item:topSelling` wiring as-is (both grew a further
~10-15 lines) rather than treat sharding either aggregator file as a
prerequisite — logged here for a future dedicated session, not
addressed. `packages/db/src/repositories/item.repository.ts` and
`apps/client/src/pages/sales/SearchSelect.tsx` both went over cap
from this session's own additions and were split before commit:
`item.repository.ts` (304→299, comments trimmed) and
`SearchSelect.tsx` (326→279, its results list/grid rendering
extracted to new `SearchSelectResults.tsx`, 93 lines).
`SearchSelect.tsx` gained two purely-additive optional props
(`initialResults`, `resultsLayout`) — both default to the prior
behavior, verified zero-impact on its other 4 callers
(`CustomerPopover`, `PurchasePage`, `JobIssuePartForm`,
`DeliveryPartLines`) by grep, none of which pass either prop.
`ItemResultRow.tsx` and `HeldSalesPopover.tsx` are both deleted
(superseded by `ItemProductCard.tsx` and `QueueStrip.tsx`
respectively) — confirmed zero remaining references by grep before
deletion.
`npm run verify` 428→434 (6 new `item.repository.test.ts` tests:
`stockOnHandMilli` real-movement-sum/null-when-none/null-when-not-
tracked, `topSellingItems` order-by-quantity/excludes-never-sold/
excludes-cancelled), confirmed after every sub-task (E-1 through
E-5), plus `npm run build --workspace=@shop/client` and
`--workspace=@shop/server` clean throughout. **Real running-window
verification, not just code-read**: same session-only
`playwright-core` precedent as prior sessions (not persisted,
`better-sqlite3` rebuilt for Electron then restored to system-Node
afterward, confirmed via `git status --short`); connected over CDP
(`chromium.connectOverCDP` against a manually-spawned
`electron.exe --remote-debugging-port`) rather than
`_electron.launch`, since the latter's own launch-line detection
failed in this sandbox for a reason not fully diagnosed (Electron
rejected a `--remote-debugging-port=0` switch Playwright inserts as
"bad option") — logged as a working fallback for any future session
hitting the same issue, not investigated further. All 7 scenarios
from the brief run against real dev-DB fixtures (`Compressor` Rs
6,000, `Compressor 2 Ton` Rs 9,000, `Khan Wholesale`): grid loaded
with 2 cards (this dev DB's only 2 items — package-icon placeholders,
item codes, "Out of stock" red badges, prices, "P" pills all
rendered correctly), typing "comp"/"Compressor 2" filtered the grid
correctly via the same `item:search` call, clicking a card opened the
inline qty editor inside the card with the qty input auto-focused and
pre-selected, typing "2"+Enter added Compressor 2 Ton × 2 = Rs 18,000
(hand-calc match) and cleared the search, Alt+H held the sale and
showed the correct chip ("Walk-in · 1 item · Rs 18,000 · Resume") in
the new bottom-of-panel queue strip, Resume restored the cart exactly,
and selecting Khan Wholesale correctly left the (retail-only, no
wholesale item_price row for this item) card prices at Rs 9,000/Rs
6,000 unchanged while the cart line's own price stayed correct. Final
query (`sale_line` grouped by `item_id`, `ORDER BY SUM(quantity)
DESC LIMIT 5`) confirmed the top result (`Compressor 2 Ton`) matched
the first card shown throughout the whole walkthrough. Zero bugs
found or introduced this session (see BUG-23 above for the one
pre-existing dead-channel finding, logged not fixed).

### BUG-23: `item:stockOnHand` channel key exists with no handler/preload/client wiring — LOW

Found in: Session 46 (POS card grid redesign), 2026-09-09, while
reading `channels.ts` before adding the new `item:topSelling` channel.
Description: `apps/server/src/ipc/channels.ts`'s `item.stockOnHand:
'item:stockOnHand'` has zero handler registration in
`item.handler.ts`, zero exposure in `preload.ts`, and zero usage
anywhere in `apps/client/src` — confirmed by grep across the whole
repo. An orphaned channel key from an earlier phase; this session's
new stock-on-hand field (`ItemDto.stockOnHandMilli`, delivered via the
existing `item:search`/new `item:topSelling` channels) does not use
or replace it.
Impact: None today — nothing calls it, so it fails silently by simply
never resolving rather than erroring. A future session that assumes
this channel is wired up (the name is exactly what a "get one item's
stock" feature would be named) would discover it does nothing.
Fix: Either wire it up if a single-item stock lookup is ever needed,
or remove the dead channel key. Not investigated further — out of
...
| 13           | Customer Ledger, Invoice Modal, Payment Receipt, Customer Statement, Add Customer, Import Balances, + polish series  | ✅ COMPLETE — all eleven tasks (CL-0a–CL-10) plus a same-day renderer-only polish series over the customer screens (phone validation, button/icon consistency, card redesigns, CSV ledger export, spacing); owner visually confirmed the running app on the shop machine 2026-09-20                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | CL-0a–CL-10 + polish series (2026-09-19/20). 602/602 tests (598 + 4 new repository tests written at close-out). Visually confirmed 2026-09-20. See `docs/phases/PHASE_13.md` §7/§9 |
| 14           | Jobs module redesign (UI/UX) — multi-technician, intake, auto-transitions, cancellation, history, + UI polish series | ✅ COMPLETE — P14-1–P14-8 (multi-tech assignment, customer intake redesign, status auto-transitions, cancellation+stock reversal, diagnosed-fault field, list search/pagination/overdue indicators, chronological History panel) + three renderer-only polish passes: V1–V5 (brand dropdown, status-picker de-emphasis, Add Part visibility, list pagination, card/shadow design consistency), F1–F3 (New Job button variant, card/shadow treatment on header/faults/sidebar, Deliver Job drawer→modal conversion, money-field sanitization), G1–G6 (removed the manual status picker and its now-redundant sidebar duplicate per owner decision, moved delivered-state info into the header card, renamed "Customer"→"Client" on job screens only, delivery modal width/button-colour pass, Create Job button variant) | P14-1–P14-8 + V1–V5 + F1–F3 + G1–G6 (2026-09-20). 627/627 tests. See `docs/phases/PHASE_14.md`                                                                                     |
...
### BUG-COMMISSION-MULTI: Multi-technician commission is not split — SUPERSEDED

Found in: Phase 14, P14-1/P14-5, 2026-09-20 (per OD-1's original scope
call — explicitly deferred from this phase, logged now that P14-5
actually shipped multi-technician assignment in the UI).
Description (as originally found): `job_technician` (P14-1) and
`TechnicianAssignmentPanel.tsx` (P14-5) let staff assign more than one
technician to a job. Commission was computed and recorded by Phase 7's
`recordCommissionIfEligible` (`job-delivery.handler.ts`) using
`job.assignedTo` only — a single technician, the fixed rate configured
on `party.commission_bp`. With two technicians assignable, that logic
had no way to split (or duplicate) a commission between them; it paid
only whichever technician `job.assignedTo` happened to point to (the
first ever assigned, per P14-1's own decision).
Status: **SUPERSEDED by ADR-0015, commit `6e480e2` (Phase 16, P16-3a
Checkpoint 2, 2026-09-24).** Phase 7's automatic per-technician
commission model — `party.commission_bp`, `recordCommissionIfEligible`,
`computeCommission` — no longer exists; the whole mechanism this bug
was reported against was deleted, not patched. Commission is now an
owner-approved claim (`commission_claim`/`commission_decision`/
`commission_decision_recipient`) configured per service charge, with
the owner naming one or more recipients (any active staff, not limited
to `job_technician`'s rows — OD-16-12) and an explicit paisa amount
each at approval time. The original "which technician gets paid"
ambiguity this bug described cannot occur under the new model — the
owner decides explicitly, every time, per claim. No further fix is
needed or possible against the old code path, since it no longer
exists.

### BUG-TECHLIST-1: Job list Technicians column shows only the primary (first-assigned) technician — LOW

Found in: Phase 14, P14-7, 2026-09-20. Owner decision (option b over
option a — a `GROUP_CONCAT` subquery was considered and explicitly
declined for now).
Description: `JobsPage.tsx`'s Technicians column (via `JobsTableRow.tsx`)
displays `job.assignedTo`'s resolved name only — the single "primary"
technician column that already existed pre-P14-5. Jobs with a second (or
third) technician assigned via `job_technician` (P14-1/P14-5) show no
indication of them in the list; the full multi-technician assignment is
only visible on the job card (`TechnicianAssignmentPanel.tsx`).
Impact: A staff member scanning the job list cannot tell at a glance
that a job has more than one technician on it. Not a data-correctness
issue — `job_technician` itself is complete and correct; this is a
display gap in the list view only.
Fix: Add a `GROUP_CONCAT` subquery on `job_technician` (filtered
`unassigned_at IS NULL`, joined to `party` for names) to `JobSummaryDto`
and `listJobsQuery`'s SELECT, when multi-tech visibility in the list
becomes operationally important enough to justify the added query
complexity.
Status: UNFIXED — accepted gap at this shop's volume, not scheduled.

### DEBT-6: `job.repository.ts` was already over the 300-line cap before Phase 14 touched it — RESOLVED

Found in: Phase 14, P14-1, 2026-09-20. Fixed: Phase 14, P14-2 (same day,
before any P14-2 UI code), per explicit owner instruction not to defer it.
Description: `packages/db/src/repositories/job.repository.ts` was already
623 lines before P14-1 — well past the 300-line project convention —
confirmed via `wc -l` before any edit. P14-1's dual-write change to
`assignTechnician` (job_technician INSERT alongside the existing
job.assigned_to UPDATE) added ~33 lines, bringing it to 656.
Fix applied: split into four files, mirroring the existing
`job-part.repository.ts`/`job-delivery.repository.ts` sibling-file
pattern already used in this directory — `job-shared.ts` (row
type/mapper, status derivation, invoice lookup, doc-number sequence),
`job-query.repository.ts` (the four read methods, as standalone
functions), `job-technician.repository.ts` (gained a new exported
`assignTechnicianWrite` function alongside its existing
`listTechnicianAssignments`), and `job.repository.ts` itself, now a
275-line shell: `createJob`/`updateJobStatus` plus thin one-line
delegators for every read method and for `assignTechnician`.
`createJob`'s hand-reconstructed return object (a near-duplicate of its
own INSERT values, ADR-3-style redundancy) was also simplified to
re-select via `JOB_RECORD_COLUMNS` + `toJobRecord`, the same pattern
`updateJobStatus`/`assignTechnician` already used — both a line-count win
and a real duplication fix, not scope creep.
Verified: `wc -l` on all four files pasted (275/198/161/139, all ≤ 300);
`npm run typecheck`/`npm run lint`/`npm run verify` all clean, 602/602;
a standalone script re-exercised all seven `JobRepositoryPort` methods
(createJob, assignTechnician ×2, updateJobStatus, getJob, listJobs,
getJobSplit, getTechnicianCustody) against a real migrated+seeded SQLite
DB after the split, output pasted in session transcript, confirming no
behaviour change.
Status: RESOLVED — commit pending in this session.

### DEBT-5: `RecordPaymentModal.tsx` (268 lines) and `LedgerExportMenu.tsx` (194 lines) approaching the 300-line cap — LOW, watch list

Found in: Phase 13 polish series close-out, 2026-09-20
Description: Both files are well under the 300-line hard limit today, but
`RecordPaymentModal.tsx` already needed one extraction this session
(`PaymentMethodToggle.tsx`, pulled out when it briefly hit 302 lines
mid-session) after several rounds of layout changes, and
`LedgerExportMenu.tsx` is a new, single-purpose file that could grow if
more export options are added later.
Impact: None yet — both build and typecheck clean today. Flagged so the
next session that touches either file checks `wc -l` first rather than
discovering the limit mid-edit.
Fix: When next editing `RecordPaymentModal.tsx`, consider extracting the
customer-context block or the amount-input group before adding anything
new. When next editing `LedgerExportMenu.tsx`, consider extracting the
custom-date-range sub-panel if more range options are added.
Status: UNFIXED — no action needed yet, watch-list only.
...
### BUG-17: `job.update` / `job.returnPart` / `job.addAccessory` do not exist — deliberate stubs, LOW — PARTIALLY RESOLVED
...
that one field.
**Update, 2026-09-22, I4:** `job.update` is now PARTIALLY RESOLVED. New
narrow `job:updateDetails` IPC (`job-details.repository.ts`,
`job-details.handler.ts`) covers exactly the fields that need no
stock/ledger write: `jobClientId`, `applianceType`, `applianceBrand`,
`applianceModel`, `applianceSerial`, `reportedFault`, `promisedDate`,
`notes` — same undefined-means-skip pattern as P14-6's
`job:updateDiagnosis`. Guarded server-side: throws
"Cannot edit a delivered or cancelled job." and writes nothing if the
job's current (derived) status is `delivered`/`cancelled`. Renderer:
Reported fault, Appliance type, Brand, and Notes are now inline-editable
on the job card (same always-editable-when-open + dirty-gated Save/Cancel
pattern as the existing Diagnosed fault field); a job with no linked
client can be linked to an _existing_ job_client via an inline
`JobClientPicker`. Promised date is unchanged — still edited via the
existing `job:updateDiagnosis` path, not duplicated. **Still open, out
of scope for I4:** `job.returnPart` (unused-parts return to stock) and
`job.addAccessory` remain unbuilt (their own scope, separate from this
narrow details-edit). Also newly out of scope, logged as a future item
this session: creating a brand-new job_client via the inline
"Link client" flow (only linking an _existing_ client is supported), and
changing an _already-linked_ client to a different one post-creation.
Credit notes and any post-delivery correction path remain entirely
...
| 0014 | Hard delete of job records is not supported and will not be built. Cancel (with reason) is the correct action for all pre-delivery job closures, including mistakes and customer withdrawals before work starts. This preserves the audit trail. A job created by mistake should be cancelled with reason "Created in error". Voiding (a softer form of cancel) is deferred to a future phase if operationally needed (see Q-VOID).                                                                                                        | 2026-09-22 |
| 0015 | Commission is an owner-approved claim (`commission_claim`/`commission_decision`/`commission_decision_recipient`/`commission_decision_reversal`), not an automatic posting. Replaces Phase 7's per-technician `party.commission_bp` model entirely — delivery writes a claim, never money, inside its own transaction; the owner approves/rejects, naming recipients; a wrong decision is corrected by reversal (`UNIQUE(claim_id, attempt_no)`, not `UNIQUE(claim_id)`), never edited. See `docs/decisions/ADR-0015-commission-claims.md`. | 2026-09-22 |
