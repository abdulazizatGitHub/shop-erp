...
Description: `job_technician` (P14-1) and `TechnicianAssignmentPanel.tsx`
(P14-5) let staff assign more than one technician to a job. Commission is
computed and recorded elsewhere (`recordCommissionIfEligible` in
`job-delivery.handler.ts`, Phase 7) using `job.assignedTo` only — a
single technician, the fixed rate configured in Settings. With two
technicians now assignable, commission logic does not know how to split
(or duplicate) a fixed-rate commission between them; it still only pays
the one technician `job.assignedTo` happens to point to (the first
technician ever assigned, per P14-1's own decision).
Impact: A second (or third) technician on a delivered job earns no
commission at all today — not a wrong number, a missing one. Not a
money-correctness bug in the sense of posting a wrong value; it's a
functionality gap in a feature (multi-tech) that didn't exist when the
commission logic was built (Phase 7).
Fix: Needs a real design decision, not a mechanical fix — OD-1 already
settled the model (each technician independently earns their own fixed
configured rate, not a percentage split), but the actual commission
write path needs to iterate `job_technician`'s active rows instead of
reading `job.assignedTo` alone. Fix location: Settings — Job
Configuration phase (future), per OD-1.
Status: UNFIXED — deliberate, owner-approved scope exclusion for Phase
14 (OD-1), not an oversight.
