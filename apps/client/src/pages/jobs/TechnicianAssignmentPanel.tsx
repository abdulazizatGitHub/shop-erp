 ...
 * job_technician (P14-1). Read-only once the job is delivered/cancelled
 * — derived directly from job.status, no new prop.
 */
export function TechnicianAssignmentPanel({
  ...
  const isReadOnly = job.status === 'delivered' || job.status === 'cancelled';
  ...
  async function handleUnassign(assignmentId: string): Promise<void> {
    ...
    setActionError(null);
    try {
      await ipc.job.unassignTechnician({ id: assignmentId });
      load();
    } catch (err) {
