...
export interface ImportSupplierBalanceInstructionsProps {
  readonly columns: readonly string[];
  readonly sampleRow: readonly string[];
}

/** Page 1 body of ImportSuppliersModal — sample-file download + the "before you import" checklist. */
export function ImportSupplierBalanceInstructions({
  columns,
  sampleRow,
}: ImportSupplierBalanceInstructionsProps): React.JSX.Element {
  return (
      ...
      <div>
        ...
        <div className="flex items-center gap-3 rounded-xl border border-line bg-surface-page px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-medium text-ink">Supplier Balances CSV</p>
            <p className="text-xs text-ink-faint">
              Defines opening balances for existing suppliers
            </p>
          </div>
          <button
            type="button"
            onClick={() => {
              downloadCsv('supplier-balances-sample.csv', columns, sampleRow);
            }}
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-start gap-2">
            ...
            <span className="text-xs text-ink-faint">
              Suppliers must already exist (matched by name)
            </span>
          </div>
          <div className="flex items-start gap-2">
            <CheckCircle size={14} strokeWidth={1.5} className="mt-0.5 shrink-0 text-success" />
