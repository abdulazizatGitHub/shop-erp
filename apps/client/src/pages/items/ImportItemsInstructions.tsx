...
export interface ImportItemsInstructionsProps {
  ...
  readonly openingStockColumns: readonly string[];
  readonly openingStockSampleRow: readonly string[];
}

/** Page 1 body of ImportItemsModal — sample-file downloads + the "before you import" checklist. */
export function ImportItemsInstructions({
  itemColumns,
  itemSampleRow,
  openingStockColumns,
  openingStockSampleRow,
          ...
          <div className="flex items-center gap-3 rounded-xl border border-line bg-surface-page px-4 py-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-ink-faint">
                Optional — imported separately, not yet part of this screen
              </p>
            </div>
            <button
              type="button"
              onClick={() => {
                downloadCsv('opening-stock-sample.csv', openingStockColumns, openingStockSampleRow);
              }}
              className="flex items-center gap-1.5 rounded-lg border border-line px-3 py-1.5 text-xs font-medium text-ink transition-colors hover:bg-surface-page"
            >
              <Download size={14} strokeWidth={1.5} />
