...
| [0012](ADR-0012-document-numbering.md)               | Document numbers are `PREFIX-NNNN` (4-digit minimum, no device code, no year). `payment` splits into `payment_in`/RCP and `payment_out`/PMT.                                     | 2026-08-28 | Accepted |
| [0013](ADR-0013-multi-unit-selling.md)               | Items may sell in a unit different from their stock unit — fixed system conversions (`uom_conversion`) and item-specific conversions (`item.alt_uom_id`/`alt_uom_factor_milli`). | 2026-08-28 | Accepted |
