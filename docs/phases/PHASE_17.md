...
approved for build. **P17-1 is DONE** (built and verified this session,
2026-09-26 — see §6/§8). P17-2 through P17-5 and P17-7 remain to build.
**Started:** 2026-09-26
**Completed:** —
**Branch:** main
...
- Rev 5 (2026-09-26, this revision): **P17-1 built and verified.** Core
  predicate (`computeNegativeStockItems`), typed errors
  (`NegativeStockBlockedError`/`NegativeStockConfirmationRequiredError`),
  the `negativeStockPolicy` setting, the new `counterStockMilli` field
  (item repository/contracts), the Option C client confirmation flow
  (`useSaleFlow.ts`), and the new Stock & Alerts settings section are all
  committed. 13 new/rewritten tests, `npm run verify` 858/858, both
  workspace builds clean, and all four §8 scenarios independently
  confirmed against the real dev DB (`data/shop-dev.db`) via a throwaway
  script (deleted after use, dev DB restored to its exact prior state).
  See §6/§8 for the full verification record.
...
**Resolved (Rev 4):** D17-1 answered the Option A/B/C choice with Option
C — see §2.1. **P17-1 is built and verified (Rev 5)** — nothing in this
phase is blocked. Remaining tasks (P17-2 through P17-5, P17-7) are ready
to start.
...
- [x] **P17-1** — DONE, 2026-09-26. new/rewritten tests, all combinations named explicitly
      (Option C, D17-1):
